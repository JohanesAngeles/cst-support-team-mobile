...
export default function AdminUniversityApplicationsScreen() {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      const { data } = await client.get('/admin/university-applications');
      setApps(data);
    } catch {
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            setActing(id);
            try {
              const { data } = await client.patch(`/admin/university-applications/${id}`, { status });
    ...
    <View style={{ flex: 1, backgroundColor: '#F5F7FA' }}>
      <SafeAreaView style={{ flex: 1 }} edges={['top']}>
        ...
        <View style={{ paddingHorizontal: 20, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: '#EBEBEF', backgroundColor: '#FFFFFF', flexDirection: 'row', alignItems: 'center', gap: 10 }}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 17, fontWeight: '800', color: '#1A1A2E' }}>University Applications</Text>
    ...
    <View style={{ backgroundColor: '#FFFFFF', borderRadius: 16, borderWidth: 1, borderColor: '#EBEBEF', overflow: 'hidden' }}>
      ...
      <View style={{ padding: 16, gap: 12 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 16, fontWeight: '800', color: '#1A1A2E' }}>{app.name}</Text>
            <Text style={{ fontSize: 13, color: '#8E8E93', marginTop: 2 }}>
              {[app.truckType, app.cdlClass && `Class ${app.cdlClass}`, app.yearsDriving != null && `${app.yearsDriving} yrs driving`].filter(Boolean).join(' · ')}
            </Text>
          </View>
        ...
        <View style={{ gap: 4 }}>
          {[
            ...
            <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              <Ionicons name={row.icon as any} size={14} color="#8E8E93" />
        ...
        {app.goals ? (
          <Text style={{ fontSize: 13, color: '#8E8E93', lineHeight: 19 }} numberOfLines={3}>{app.goals}</Text>
        ) : null}
