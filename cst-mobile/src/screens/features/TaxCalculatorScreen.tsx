...
export default function TaxCalculatorScreen() {
  ...
  const buildTaxHTML = (report: any) => {
    ...
    <div class="subtitle">Tax Year ${year} &nbsp;·&nbsp; Generated ${new Date().toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})} &nbsp;·&nbsp; Road Ready Network</div>
    <div class="grid">
    ...
    <div class="disclaimer"><strong>Disclaimer:</strong> These figures are estimates based on data entered in the Road Ready app. Self-employment tax rate used: 14.13%. Federal income tax rate used: 22%. Consult a licensed CPA or tax professional before filing. IFTA taxes are filed separately through your base jurisdiction.</div>
    <div class="footer">Generated by Road Ready Network &nbsp;·&nbsp; Not a substitute for professional tax advice</div>
    </body></html>`;
  };
  ...
  const handleExport = async () => {
    try {
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(uri, { mimeType: 'application/pdf', dialogTitle: `Road Ready Tax Report ${report.year}` });
      } else {
